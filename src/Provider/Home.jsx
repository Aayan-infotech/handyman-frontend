...
const handleAllData = async () => {
  ...
  try {
    ...
    if (getProviderJobs.fulfilled.match(result)) {
      setData(result.payload?.data || []);
      addAlert("Nearby Jobs Fetched Successfully", "success");
    } else {
      throw new Error(result.payload?.message || "Error fetching jobs.");
    }
  } catch (error) {
...
const fetchNotifications = async () => {
  ...
  try {
    const url = `http://3.223.253.106:7777/api/notification/getAll/${userType}/${userId}`;
    const response = await axios.get(url);
    const newNotifications = Array.isArray(response.data.data) ? response.data.data : [];
...
const fetchMassNotifications = async () => {
  try {
    const url = `http://3.223.253.106:7777/api/massNotification/?userType=${userType}`;
    const response = await axios.get(url);
    const newMassNotifications = Array.isArray(response.data) ? response.data : [];

    setMassNotifications(newMassNotifications);
    addAlert(`You have ${newMassNotifications.length} mass notifications`, "info");
  } catch (error) {
  ...
  <div className="bg-second py-3">
    <div className="container top-section-main">
      ...
      <div className="row mt-4 gy-4 management">
        {loading ? (
          [...Array(2)].map((_, index) => (
            <Skeleton key={index} sx={{ height: 100, width: "100%" }} animation="wave" />
          ))
        ) : data.length === 0 ? (
          <div className="d-flex justify-content-center">
            <img src={noData} alt="No Data Found" className="w-nodata" />
          ...
          data.map((job) => (
            <div className="col-lg-12" key={job._id}>
              <Link to={`/provider/jobspecification/${job._id}`} className="card border-0 rounded-3 px-4">
                <div className="card-body">
                  <div className="row gy-4 gx-1 align-items-center">
                    {guestCondition ? (
                      <div className="col-lg-4">
                    ...
                    {!guestCondition ? (
                      <div className="col-lg-2">
                        <Button variant="contained" className="custom-green bg-green-custom rounded-5 py-3 w-100">
                          Contact
                        </Button>
                      </div>
                    ) : null}
